...
void Get_decomposed_ene(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  */
  ...
  for (global_num=1; global_num<=atomnum; global_num++){
    ...
    ID = G2ID[global_num];
    if (myid==ID){
      printf("Get dec energy test for atom %d start at processor %d\n",global_num,ID);
      local_num = F_G2M[global_num];
      ...
      printf("Part test pass for atom %d at processor %d\n",global_num,ID);
    }
    ...
    if (myid==Host_ID){
      ...
      fprintf(fp,"%s ",SpeName[species]);
      fprintf(fp,"%8.6f\n", Dec_tot[global_num]);
      printf("All test Pass of atom %d at processor %d\n",global_num,ID);
    }
...
void para_matrix_gen(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  if (myid==Host_ID){
    ...
    if (iter==1){
      ...
      for(j=1;j<=atomnum;j++){
        nei_num = FNAN[j];
        final_model[j] = (double*)malloc(sizeof(double)*(nei_num*Max_order+1));
        memset(final_model[j],0,(nei_num*Max_order+1)*sizeof(double));
      }
  ...
  if (myid==Host_ID){
    ...
    if (iter<=train_iter){
      ...
      n = 5; //nei_num*Max_order+Max_order*angular_num[i]; This part should be included in for i< atomnum, nei_num should change with i
      nrhs = 1;
      lda = 5; //nei_num*Max_order+Max_order*angular_num[i];
      ldb = 5; //nei_num*Max_order+Max_order*angular_num[i];
      lwork = 5; //nei_num*Max_order+Max_order*angular_num[i]+1;
