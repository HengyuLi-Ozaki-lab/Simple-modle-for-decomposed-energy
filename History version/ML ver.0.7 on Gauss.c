...
void Get_decomposed_ene(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  for (global_num=1; global_num<=atomnum; global_num++){
    ...
    ID = G2ID[global_num];
    if (myid==ID){
      printf("Get dec energy test start at processor %d\n",ID);
      ...
      printf("Add test pass at processor %d\n",ID);
      ...
      printf("Send test pass at processor %d\n",ID);
    }
    ...
    if (myid==Host_ID){
      ...
      fprintf(fp,"%8.6f\n", Dec_tot[global_num]);
      printf("Receive test Pass at processor %d\n",ID);
    }

    if (myid==ID){
      printf("Get decomposed_ene Pass atom %d at processor %d\n",global_num,ID);
    }
  }
  fclose(fp);
}
...
void para_matrix_gen(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  if (myid==Host_ID){
    ...
    if (iter==1){
      ...
      for(j=1;j<=atomnum;j++){
        nei_num = FNAN[j];
        final_model[j] = (double*)malloc(sizeof(double)*(nei_num*Max_order+Max_order*angular_num[j]+1));
        memset(final_model[j],0,(nei_num*Max_order+Max_order*angular_num[j]+1)*sizeof(double));
      }
  ...
  if (myid==Host_ID){
    ...
    if (iter<=train_iter){
      ...
      for (i=1;i<=atomnum;i++){
        ...
        nei_num = FNAN[i];
        n = nei_num*Max_order+Max_order*angular_num[i]; //nei_num*Max_order+Max_order*angular_num[i]; This part should be included in for i< atomnum, nei_num should change with i
        nrhs = 1;
        lda = nei_num*Max_order+Max_order*angular_num[i]; //nei_num*Max_order+Max_order*angular_num[i];
        ldb = nei_num*Max_order+Max_order*angular_num[i]; //nei_num*Max_order+Max_order*angular_num[i];
        lwork = nei_num*Max_order+Max_order*angular_num[i]; //nei_num*Max_order+Max_order*angular_num[i]+1;
      ...
      for (i=1;i<=atomnum;i++){
        ...
        for (j_1=1;j_1<=nei_num-1;j_1++){
          for (k=j_1+1;k<=nei_num;k++){
            for (p_1=1;p_1<=Max_order;p_1++){
              energy_test = constant_matrix[i][count_para]*cut_off(dis_nei[i][j_1],r_cut,0)*cut_off(dis_nei[i][k],r_cut,0)*pow(ang_nei[i][count_ang],p_1-1);
              model_energy[i][iter] += energy_test;
              count_para ++;
            }
            count_ang ++;
    ...
    if (iter==train_iter){
      for (i=1;i<=atomnum;i++){
        nei_num = FNAN[i];
        for (j=1;j<=(nei_num*Max_order+Max_order*angular_num[i]);j++){
    ...
    if (iter>train_iter){
      for (i=1;i<=atomnum;i++){
        ...
        for (j=1;j<=nei_num;j++){
          for (p=1;p<=Max_order;p++){
            energy_test = final_model[i][count_para]*cut_off(dis_nei[i][j],r_cut,0)*pow(dis_nei[i][j],p-1);
            model_energy[i][iter] += energy_test;
            count_para ++;
          }
        }
        ...
        for (j_1=1;j_1<=nei_num-1;j_1++){
          for (k=j_1+1;k<=nei_num;k++){
            for (p_1=1;p_1<=Max_order;p_1++){
              energy_test = final_model[i][count_para]*cut_off(dis_nei[i][j_1],r_cut,0)*cut_off(dis_nei[i][k],r_cut,0)*pow(ang_nei[i][count_ang],p_1-1);
              model_energy[i][iter] += energy_test;
              count_para ++;
            }
            count_ang ++;
