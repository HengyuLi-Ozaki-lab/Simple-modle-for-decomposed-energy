...
void para_matrix_gen(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  if (myid==Host_ID){
    ...
    n = 4; //nei_num*Max_order+Max_order*angular_num[i]; This part should be included in for i< atomnum, nei_num should change with i
    nrhs = 1;
    lda = 4; //nei_num*Max_order+Max_order*angular_num[i];
    ldb = 4; //nei_num*Max_order+Max_order*angular_num[i];
    ...
    if (iter==MD_IterNumber){
      for (i=1;i<=atomnum;i++){
        ...
        for (j=1;j<=MD_IterNumber;j++){
          fprintf(fp3,"%8.6f ",model_energy[i][j]);
        }
        fprintf(fp3,"\n");
      }
