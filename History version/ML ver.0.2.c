...
void para_matrix_gen(int iter,char filepath[YOUSO10],char filename[YOUSO10])
{
  ...
  int i,j,k,p,j_1,j_2,k_1,p_1,p_2,nei_num,count_ang,count_ang1,count_ang2,row,row_mpi,column,column_mpi,species,myid,ID;
  int parameter_count,constant_count,proc_count,matrix_count,test,count_para;
  double r_cut,loss,energy_test,matrix_test;
  ...
  if (myid==Host_ID){
    ...
    if (iter==1){
      ...
      matrix_a = (double***)malloc(sizeof(double**)*(atomnum));
      for (i=1; i<=atomnum; i++){
        matrix_a[i] = (double**)malloc(sizeof(double*)*(FNAN[i]*Max_order)); 
      ...
      matrix_b = (double***)malloc(sizeof(double**)*(atomnum));
      for (i=1; i<=atomnum; i++){
        matrix_b[i] = (double**)malloc(sizeof(double*)*(FNAN[i]*Max_order)); 
        for (j=1; j<=(Max_order*FNAN[i]); j++){
      ...
      matrix_c = (double**)malloc(sizeof(double*)*(atomnum));
      for(j=1;j<=atomnum;j++){
        matrix_c[j] = (double*)malloc(sizeof(double)*(FNAN[j]*Max_order+1));
        memset(matrix_c[j],0,(FNAN[j]*Max_order+1)*sizeof(double));
      }

      matrix_a_ = (double***)malloc(sizeof(double**)*(atomnum));
      for (i=1; i<=atomnum; i++){
        matrix_a_[i] = (double**)malloc(sizeof(double*)*(Max_order*angular_num[i])); 
        for (j=1; j<=FNAN[i]*angular_num[i]; j++){
          matrix_a_[i][j] = (double*)malloc(sizeof(double)*(FNAN[i]*Max_order+1));
          memset(matrix_a_[i][j],0,(FNAN[i]*Max_order+1)*sizeof(double));
        }
      }

      matrix_b_ = (double***)malloc(sizeof(double**)*(atomnum));
      for (i=1; i<=atomnum; i++){
        matrix_b_[i] = (double**)malloc(sizeof(double*)*(Max_order*angular_num[i])); 
        for (j=1; j<=FNAN[i]*angular_num[i]; j++){
      ...
      matrix_c_ = (double**)malloc(sizeof(double*)*(atomnum));
      for(j=1;j<=atomnum;j++){
      ...
      if (parameter_matrix!=NULL){
        printf("Is not empty\n");
        exit(0);
      }

      parameter_matrix = (double**)malloc(sizeof(double*)*(atomnum));
      for(j=1;j<=atomnum;j++){
        parameter_matrix[j] = (double*)malloc(sizeof(double)*(pow((nei_num*Max_order+Max_order*angular_num[j]),2)+1));
        memset(parameter_matrix[j],0,(pow((nei_num*Max_order+Max_order*angular_num[j]),2)+1)*sizeof(double));
      }

      if (constant_matrix!=NULL){
        exit(0);
      }

      constant_matrix = (double**)malloc(sizeof(double*)*(atomnum));
      for(j=1;j<=atomnum;j++){
  ...
  if (myid==Host_ID){
    ...
    n = nei_num*Max_order+Max_order*angular_num[i];
    nrhs = 1;
    lda = nei_num*Max_order+Max_order*angular_num[i];
    ldb = nei_num*Max_order+Max_order*angular_num[i];
    lwork = nei_num*Max_order+Max_order*angular_num[i];
